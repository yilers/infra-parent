...
当前已经实现第三方认证平台配置管理、已登录用户绑定与解绑钉钉账号，以及使用已绑定的钉钉身份直接登录UPM。
不自动创建用户，也不在登录流程中同步组织架构。
...
生产环境使用前，建议再增加独立的密钥加密组件；加密实现不能复用业务应用的 SSO 客户端密钥作为固定明文密钥。
...
登录接口位于 `/auth/third`，不要求用户已经登录：

| 接口 | 说明 |
| --- | --- |
| `POST /authorize` | 按租户编码生成五分钟有效的登录 state 和钉钉授权地址 |
| `POST /login` | 一次性消费 state，使用授权码查询钉钉身份并签发UPM token |

登录流程如下：

1. 登录页从 `tenantCode` 查询参数确定租户，未提供时使用默认租户编码 `yilers.com`。
2. 后端通过租户编码查询已启用的钉钉配置，把租户 ID、平台和设备类型写入 Redis 一次性 state。
3. 授权回调页根据 `login_` state 前缀返回登录页，前缀只用于前端路由分流。
4. 后端消费 state，获取钉钉用户身份，并在 state 对应租户内查询绑定关系。
5. 未绑定时明确拒绝登录，不根据钉钉信息自动创建UPM用户。
6. 绑定存在时复用账号密码登录的租户、账号和应用状态校验，由 Sa-Token 签发原有UPM token。

其他租户使用独立登录入口时，应显式传入租户编码：

```text
https://upm.example.com/#/auth/login?tenantCode=example.com
```
...
1. 在测试组织中完成钉钉直接登录回归测试。
2. 生产使用前对 Client Secret 增加可更换的加密存储。
3. 如确有需要，再单独设计部门和用户同步任务，不能把组织同步混入登录回调。
