...
新租户以租户 1 为模板复制应用、菜单和租户管理员授权关系，只创建租户管理员。平台管理员只存在于租户 1。应用的 SSO 密钥、回调地址、推送地址和启用状态不会复制，新租户必须自行完成客户端配置。

平台管理员可以预览并同步租户 1 后续发生的应用菜单变化，具体规则见 [TENANT_PERMISSION_SYNC.md](./TENANT_PERMISSION_SYNC.md)。
